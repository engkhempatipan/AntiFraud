...
public class PlayIntegrityMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main_integrity);
    ...
    private class getTokenResponse extends AsyncTask<String, Integer, String[]> {
        ...
        protected String[] doInBackground(String token) throws Exception {
            ...
            Request request = new Request.Builder()
                    .get()
                    .url("https://play-integrity-checker-server-silk-mu.vercel.app" + "/api/check?token=" + token)
                    .build();
    ...
    private void showErrorDialog(String title, String message) {
        new MaterialAlertDialogBuilder(PlayIntegrityMainActivity.this, R.style.Theme_PlayIntegrityAPIChecker_Dialogs)
                .setTitle(title)
                .setCancelable(true)
    ...
    private void setIcon(ImageView img, int state) {
        if (state == -1) {
            img.setImageDrawable(ContextCompat.getDrawable(PlayIntegrityMainActivity.this, R.drawable.ic_unknown));
            img.setContentDescription(getString(R.string.status_unknown));
        } else if (state == 0) {
            img.setImageDrawable(ContextCompat.getDrawable(PlayIntegrityMainActivity.this, R.drawable.ic_fail));
            img.setContentDescription(getString(R.string.status_fail));
        } else {
            img.setImageDrawable(ContextCompat.getDrawable(PlayIntegrityMainActivity.this, R.drawable.ic_pass));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.about) {
            new AboutDialog(PlayIntegrityMainActivity.this).show();
            return true;
